...
var civilianFactory = {
    getSourceId: function (roleType, room) {
        ...
        sourcesAccessPoints = sourcesAccessPoints.map(s => {
            return { 
                ...s, 
                creepCount: Math.round(_.find(roles.getRoles(),r => r.type == roleType).count / (totalAps / s.ap)) 
            }
        });
        for (let source of sourcesAccessPoints) {
    ...
    run: function (spawner) {
        ...
        prioritisedRoles = _.filter(prioritisedRoles, role => _.filter(Game.creeps, (creep) => creep.memory.role == role.type).length < role.count)
